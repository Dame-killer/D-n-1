            ...
            <div class="modal-content">
                ...
                <form action="{{ route('aa-css-create') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="modal-body">
                            <select name="name" style="margin-top: 20px">
                                <option>Chọn sinh viên</option>
                                @foreach ($users as $user)
                                    <option  value="{{ $user->id }}">{{ $user->name }}-{{ $user->student_code }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="modal-body">
                            <select name="subject_name" style="margin-top: 20px">
                                ...
                                @foreach ($class_subjects as $class_subject)
                                    <option value="{{ $class_subject->cs_id }}">
                                        {{ $class_subject->class_name }}K{{ $class_subject->school_year }} -
